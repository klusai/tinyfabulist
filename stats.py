    ...
    for model, scores in score_totals.items():
        ...
        if count > 0:
            ...
            averages[model] = {
                ...
                "average_score_(mean)": (grammar + creativity + moral_clarity + adherence) / 4,
...
def generate_plotly_visualizations(averages, tf_model_stats, stats_folder, timestamp, output_mode):
    """
    ...
    for i, metric in enumerate(performance_metrics):
        ...
        fig.add_trace(
            go.Bar(
                ...
                textposition='auto',
                marker_color=perf_colors.get(color_key, f'hsl({50 + i * 70}, 70%, 50%)'),
...
def generate_matplotlib_visualizations(averages, tf_model_stats, stats_folder, timestamp, output_mode):
    """
...
def process_eval_files(eval_files, tf_model_stats, stats_folder, timestamp, output_mode, plot_mode="plotly"):
    ...
    score_totals = parse_evaluation_data(eval_files)
    averages = compute_averages(score_totals)
    ...
    if plot_mode.lower() == 'plotly':
        generate_plotly_visualizations(averages, tf_model_stats, stats_folder, timestamp, output_mode)
    else:  # Use matplotlib as default or fallback
        generate_matplotlib_visualizations(averages, tf_model_stats, stats_folder, timestamp, output_mode)
...
def plot_model_averages(args):
    ...
    if eval_files:
        process_eval_files(eval_files, tf_model_stats, stats_folder, timestamp, args.output_mode, args.plot_mode)
    else:
