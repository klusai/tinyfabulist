...
console = Console()
DEEPL_SCORE = 4.59
GEMMA3_12B_SCORE = 4.43

def main(file_path="tinyfabulist/estimate/config.yaml", CACHING_RATE=0.0):
    ...
    for model in config["models"]:
        ...
        estimated_total_cost = input_cost_per_million_tokens * UNCACHED_INPUT_TOKENS + output_cost_per_million_tokens * TOTAL_OUTPUT_TOKENS
    ...
    table_data.append((
        ...
        f"~${euro_llm_cost}",
        f"{euro_llm_score}",
        f"{euro_llm_cost/euro_llm_score:.2f}" if euro_llm_score != "-" else "-"
    ))

    # Sort by score (descending, highest scores first)
    def get_score_for_sorting(row):
        score = row[4]  # Score is the 5th column (index 4)
        if score == "-":
            return -1  # Put entries without scores at the end
        try:
            return float(score)
        except (ValueError, TypeError):
            return -1

    table_data.sort(key=get_score_for_sorting, reverse=True)
