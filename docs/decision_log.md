...
Our evaluation process begins by presenting the SLM with prompts based on a structured set of predefined elements, including characters, traits, settings, conflicts, resolutions, and morals. 

Each prompt is designed to offer a narrative scaffold for the SLM to complete, ensuring consistency while allowing for creative latitude. The SLM is tasked with generating a complete fable that adheres to the given structure, integrating the specified elements into a coherent and imaginative narrative.

To evaluate the outputs generated by the SLM, we employ GPT-4 as an automated assessor. For each generated fable, GPT-4 provides a detailed evaluation based on four key dimensions: grammar, creativity, consistency with the input prompt, and plot coherence. In addition, GPT-4 is asked to infer the likely age group of the hypothetical author of the story, offering insights into the linguistic and narrative maturity of the generated text. For example, GPT-4 might determine that a fable is grammatically accurate and creatively engaging but lacks full integration of the moral, resulting in scores such as 9/10 for grammar, 8/10 for creativity, 10/10 for consistency, and 8/10 for plot coherence.
...
Our approach is inspired by existing evaluation frameworks, such as Themis, which assesses moral reasoning in language models by comparing their outputs against human ethical judgments. Themis provides a foundation for evaluating alignment with human values and ethical principles. However, our framework extends this concept by focusing on narrative consistency and creative storytelling, leveraging GPT-4's capabilities to evaluate linguistic accuracy and plot coherence alongside creative and thematic fidelity. For further details, see Themis: A Dataset and Benchmark for Evaluating Moral Alignment in Language Models (https://arxiv.org/pdf/2406.18365).
...
In summary, this evaluation framework introduces a structured and scalable approach to assess the creative and interpretive capabilities of small language models. By integrating GPT-4 as an evaluator, the system aligns its assessments with human-like judgments, ensuring that the generated fables are not only grammatically accurate but also narratively compelling and thematically aligned with the intended prompts. This approach contributes to advancing methodologies for evaluating generative language models in creative domains, highlighting the importance of narrative consistency, moral reasoning, and linguistic maturity in fable generation.